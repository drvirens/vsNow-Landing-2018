//
//  VSSmallItemCollectionViewCell.m
//  vsNow-Landing
//
...
@interface VSSmallItemCollectionViewCell ()
@property (nonatomic) VSSmallCategorytemView* itemView;
@property(nonatomic, assign) BOOL hasAppliedConstraints;
@end

@implementation VSSmallItemCollectionViewCell
...
- (void)prepareForReuse {
	...
	self.itemView.imageViewIcon.image = nil;
...
//	self.layer.borderColor = [VSLandingTheme categoryItemViewBorderBorderColor].CGColor;
  //self.itemView.imageViewIcon.image = nil;
	self.itemView.imageViewIcon.backgroundColor = [UIColor clearColor];
}
...
- (VSSmallCategorytemView*)itemView {
	if (!_itemView) {
		VSSmallCategorytemView* iv = [[VSSmallCategorytemView alloc] init];
